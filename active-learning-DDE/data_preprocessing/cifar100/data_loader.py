...
def aload_partition_data_cifar100(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
...
def load_partition_data_cifar100(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size, samplefile=None):
    BB = batch_size
    if samplefile is not None:
        transform_train, transform_test = _data_transforms_cifar100()
        labeled_set, unlabeled_set = samplefile

        train_dataset = CIFAR100_truncated(data_dir, train=True, transform=transform_train, download=True)
        test_dataset = CIFAR100_truncated(data_dir, train=False, transform=transform_test, download=True)
...
def load_partition_data_cifar100_al(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size, samplefile=None):
    BB = batch_size
    if samplefile is not None:
        transform_train, transform_test = _data_transforms_cifar100()
